...
Gin starts in `backend/cmd/server/main.go`, applies global middleware in the order `middleware.RequestID`, optional `HTTPMetrics`, `middleware.Recovery`, `middleware.RequestLogger`, `middleware.ErrorHandler`, then `cors.New(...)`, and registers operational/static routes (including `/healthz`, `/readyz`, and opt-in `/metrics`) before the `/api/v1` groups. The request ID middleware preserves or generates `X-Request-ID`, writes it to Gin context and the response header, and lets request/panic logs correlate to the same value. Generated strict handlers are mounted via `genapi.RegisterHandlersWithOptions(...)` with per-operation middleware for auth and item routes; manual Gin endpoints (like upload) stay in `backend/internal/handler/` with their own routing. Public auth endpoints use the public rate limiter, protected routes use JWT auth plus user rate limiting, services orchestrate use cases after middleware unwinds, repositories talk to models, and the shared `*gorm.DB` connection comes from `backend/internal/config/database.go` during startup.
