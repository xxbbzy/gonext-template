...
7. After contract changes, run `make gen-types` to refresh the generated frontend types (this is the standard command for those artifacts) and run `make gen` when every derived artifact must be regenerated.
